// Adapted from comments under https://www.youtube.com/watch?v=OvM6eAh8wBc
//
#include "solution.hpp"
#include <cassert>

// Transpose a collection of sequences to a sequence of simd vectors.
VideoSolution::simd_sequence_t
VideoSolution::transpose(std::vector<sequence_t> const &sequences) {
...
// pairs.
result_t
VideoSolution::compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  for (unsigned col = 1; col <= trSeq2.size(); ++col) {
    ...
    for (unsigned row = 1; row <= trSeq1.size(); ++row) {
      ...
      for (size_t k = 0; k < sequence_count_v; ++k)
        best_cell_score[k] +=
            (trSeq1[row - 1][k] == trSeq2[col - 1][k] ? match[k] : mismatch[k]);
      for (size_t k = 0; k < sequence_count_v; ++k) {
